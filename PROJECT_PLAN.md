...
### [2026-05-31] U4 施工 —— 灵感版块 iOS UI + Main 角块 + 第 5 个 Tab 接线

- **富文本 API 结论（沿用 U3，验证）——【原生富文本可用，不走 markdown 回退】**：iOS 26 与 macOS 26 同属一套 SDK（Xcode 26.5，iOS deploy target 26.0）。施工首步用同一套原生 API 实现编辑器，由 iOS App target `xcodebuild` build 作为最终验证（SwiftUI body type-check + 富文本 API 可用性只有 App target build 暴露）——`TextEditor(text: $attributedString, selection: $selection)` + `transformAttributes(in:&selection)` + `replaceSelection(&selection, withCharacters:)` 在 iOS 26 **编译通过、BUILD SUCCEEDED**。结论：**iOS 直接走原生富文本**，无需 markdown 回退。**真机运行手感（属性是否真落进 AttributedString、TextEditor 富文本渲染）须出包验证**——build 不覆盖运行期。
- **变更内容**：
  - 新增 `LinoJ-iOS/LinoJ-iOS/Screens/Inspiration/InspirationView_iOS.swift`：`NavigationStack` 单列 push 导航。根 = 大标题「灵感」(`navigationTitle`，iOS 自动 34pt 大标题) + 计数副标「X notes」+ chip 样式搜索框（绑 `vm.searchText`，带清空按钮）+ note 卡列表（白卡 14pt radius + 0.5px border + pin 标记 + `displayTitle` 16pt/600 + 正文摘要 inkSoft 单行截断 + `updatedAt` mono 时间）+ 空状态 `EmptyState(.inboxZero)` + CTA。导航栏 trailing「+ New note」按钮（`square.and.pencil`）→ `createNote()` → push 编辑器。**不复用全局 Quick Add `+`**。
    - note 行用 **iOS swipe actions**：`.swipeActions(edge: .leading)` 置顶/取消置顶（inkSoft tint）、`.swipeActions(edge: .trailing, role: .destructive)` 删除（系统红）。删除前若该 note 正被 push 编辑则先清 `pushedNoteID`。
    - push 目标拆独立 `private struct NoteEditorScreen_iOS`（持 `NoteEditorViewModel`，`navigationDestination(item: $pushedNoteID)` 绑 UUID，note 删除后失效自动 pop 回列表）：富文本工具条（加粗/斜体/标题/项目符号/勾选清单，与 macOS 同实现：`transformAttributes` 改 font / `replaceSelection` 插「• 」「☐ 」前缀）+ `TextEditor(text:$AttributedString, selection:)` + 导航栏 trailing `⋯` 菜单（`ellipsis.circle`，Pin/Unpin/Delete）。删除二次确认抽 `private struct NoteDeleteConfirmModifier_iOS: ViewModifier`（CLAUDE.md 坑），confirm message/按钮复用 W4 的 `Event.deleteConfirmMessage`/`...Confirm`/`QuickAdd.cancel`。「编辑即存」同 macOS：`.onChange(of:text)` 写回 VM（setter 重写 updatedAt）、`.onDisappear` `save()` 兜底。
  - 改 `RootTabView.swift`：`TabView` 加第 5 个 `Tab(value: .inspiration){ InspirationView_iOS() } label:{ Label(Tab.inspiration, systemImage:"lightbulb") }`，照搬既有 4 个 Tab 写法。**用 iOS 26 原生 Liquid Glass tab bar 自动容纳**，未自渲 capsule、未 `.toolbar(.hidden, for:.tabBar)`（避免两条 bar 叠加，CLAUDE.md 坑）。
  - 改 `MainView_iOS.swift`：加 `@Query var notes:[Note]` + 自持轻量 `@State noteListVM: NoteListViewModel`（`.task` 初始化，仅用 `recentNotes(limit:3)`/`createNote()`，**不污染 `MainViewModel`**）。单列 ScrollView 在 Projects section 之后、**底部 100pt 留白之前**（**未动该留白**，U8 负责），插入 `recentInspirationSection()`：小节头「最近灵感」(`Inspiration.recent`，复用 `sectionHeader`) + 横向 ScrollView 1-3 条 note mini 卡（150×86 白卡，点击 `router.current = .inspiration`）+「随手记一条」入口卡（`Inspiration.quickJot`，chip 底 + 虚线边，点击 `noteListVM.createNote()` + 切 tab）。无 note 时只显示「随手记一条」单卡。
  - **body type-check 防超时**：MainView_iOS body 加 2 条 note `@Query` 失效 `.onChange` 后触发 `unable to type-check this expression in reasonable time`（报错落在无辜 `.task` 行 62）。按 CLAUDE.md 把两条 note 失效观察抽成 `private struct NoteInvalidationModifier_iOS: ViewModifier`（吃 `noteCount`/`noteUpdatedAts`/`onChange`），body 改挂 `.modifier(...)`，恢复编译。
- **新增本地化 key**：**无**（全部复用 U0/U3 已加的 `Tab.inspiration`/`Inspiration.title`/`newNote`/`notesCount`/`searchPlaceholder`/`titlePlaceholder`/`recent`/`quickJot`、`Note.pin`/`unpin`/`delete`/`deleteConfirmTitle`、`Format.*`、`Event.deleteConfirm*`、`QuickAdd.cancel`）。故测试数不变（**192 → 192**），未加 iOS 专属文案。
- **偏离说明**：① 富文本**未走** markdown 回退——原生 API 在 iOS 26 验证可用（BUILD SUCCEEDED），按 plan「可用则用原生」分支，非偏离（与 U3 同结论）。② 项目符号/勾选清单用行前缀「• 」/「☐ 」而非 `AttributedString` 列表/勾选属性——与 U3 同一 MVP A 方案（可编辑 TextEditor 不交互渲染 `PresentationIntent` 列表），数据契约 `Note.bodyData` 不破，记为实现手段偏离。③ Main「最近灵感」section 位于 Main 内容 ScrollView（受既有 `isEmpty` 门控：开放 todos+events 全为 0 时 Main 显示 EmptyState，该 section 不出现，与 macOS 右栏只在有内容时显示同语境）——符合 plan「在 Projects 之后插入 section」，非偏离。④ 新建/置顶/删除入口用导航栏按钮 + swipe actions（plan 给的「导航栏 trailing 或列表顶 ink 按钮」二选一里选导航栏 trailing），符合 plan。
- **验收**：`swift test --package-path Packages/LinoJCore` **192 全绿**（不变，无新文案）；`swift build -Xswiftc -warnings-as-errors` **0 warning**；**iOS App target `xcodebuild ... LinoJ-iOS -destination 'generic/platform=iOS Simulator' build` BUILD SUCCEEDED**（`InspirationView_iOS.swift` + `MainView_iOS.swift` + `RootTabView.swift` 均编译过，无 body type-check 超时，0 新增代码 warning；仅有无关的 appintentsmetadataprocessor 提示）；**macOS App target build 仍 BUILD SUCCEEDED**（U4 仅改 iOS 三文件，未碰 macOS / 共享层；macOS `RootWindow.swift:416 reposition()` 警告为既有、与 U4 无关）。
- **headless 测不到、须模拟器/真机截图验证**：① 底部原生 tab bar 出现第 5 个「灵感」tab（lightbulb + label），**单条 Liquid Glass 不叠加两条 bar**；② 点 note push 富文本编辑器手感（加粗/斜体/标题落进 AttributedString 后 TextEditor 渲染、返回列表该 note 跳顶、重启仍在内容保留）；③ swipe 删除（左滑红）/ 置顶（右滑）交互；④ Main 单列底部「最近灵感」section 显示、点 mini 卡切 tab、「随手记一条」切 tab + 打开新编辑器的实际跳转。
- **影响范围**：Phase U4（iOS）。新增 `Screens/Inspiration/InspirationView_iOS.swift`；改 `RootTabView.swift`（第 5 Tab）、`MainView_iOS.swift`（角块 + notes Query + 轻量 VM + 防超时 modifier）。**共享层未改**（U2 已就绪）。**未改 macOS、entitlements、pbxproj 版本号、Widget**；**未动 MainView_iOS 底部 100pt 留白**（U8 负责）；**未 bump 版本号**（U10 做）。
- **[2026-05-31 定点修复]** 主控模拟器实测发现 iOS「+ 新笔记」入口（原 `ToolbarItem(.topBarTrailing)`）被 `RootTabView` 全局 `FloatingActions()`（`.overlay(alignment:.topTrailing)`，覆全 TabView）在右上角遮挡 → 无法新建笔记。修复：将该入口由导航栏 trailing 改为**列表顶醒目 ink 按钮**（搜索框下方、note 列表之上，对齐 macOS `InspirationView_macOS` 的「+ New note」ink 按钮，`Color.lj.ink` 底 / `Color.lj.bg` 白字 / 圆角 10），删除 `.toolbar` 块。这是 plan U4「导航栏 trailing 或列表顶 ink 按钮 二选一」里的备选放法，**行为完全不变**（仍 `createAndPush` → push 进 `NoteEditorScreen_iOS`，`pushedNoteID` / `navigationDestination` 路径不动）。仅改 `InspirationView_iOS.swift` 一个文件；未碰全局 FloatingActions、swipe/编辑器/搜索/Main 角块。iOS App target BUILD SUCCEEDED、macOS App target BUILD SUCCEEDED、`swift test` 192 全绿、`-warnings-as-errors` 0 warning。

### [2026-05-31] U5 施工 —— 重叠事件呈现：共享列分配算法 + macOS 并排渲染 + iOS 冲突提示

- **变更内容**：
  - **共享算法（`CalendarViewModel.swift`）**：新增纯函数 `static func computeOverlapLayout(events: [Event]) -> [UUID: (column: Int, columnCount: Int)]` + 实例包装 `func overlapLayout(forDay dayStart: Date) -> ...`（取 `eventsByDay[dayStart]` 调 static 版）。static 版**不读 `tick`、不 mutate、不依赖 SwiftData**，单测直接调。算法：① 按 start 升序（同 start 按 end 升序，保确定性）；② 传递性归簇——顺序扫描维护「簇内最大 end」，下个事件 `start < 簇内最大 end` 即并入（区间相交、端点相接不算），否则封闭当前簇开新簇；③ 簇内贪心分配「最左可用列」（列上一个事件 `prevEnd <= cur.start` 即可复用），`columnCount` = 该簇最大列数，**簇内所有事件 columnCount 一致**。归簇 + 列分配抽 `private static func assignColumns(...)`。
  - **macOS 渲染（`CalendarView_macOS.swift`）**：`eventLayout` 扩签名为 `eventLayout(event:columnWidth:column:columnCount:)`；`dayColumn` 渲染前对该天调一次 `vm.overlapLayout(forDay: dayStart)` 拿 map，`ForEach` 内按 `overlap[event.id] ?? (0,1)` 取 slot 传入。`laneWidth = (columnWidth-8)/CGFloat(columnCount)`、`x = 4 + CGFloat(column)*laneWidth`、`width = laneWidth - 2`（列间隙）；y/height 时间映射不变。MVP 等宽，未做向右扩展吃空列。
  - **iOS 冲突提示（`CalendarView_iOS.swift`）**：`dayEvents` 对当天调 `vm.overlapLayout(forDay:)`，`columnCount > 1` 即冲突；在 `EventCard(.iosFull)` **上方**（外层 VStack 插一行，不改 EventCard 内部）加冲突 chip `conflictChip(otherCount:)`：三角警示 SF Symbol + 「与 N 个日程重叠」（N = `columnCount-1`），`Color.lj.inkMute` 中性色 + `Color.lj.chip` capsule 底，**不用蓝**。
  - **本地化（三轨三步）**：`Localizable.xcstrings` 加 `Calendar.overlapsCount`（"Overlaps %d" / "与 %d 个日程重叠"，按位置插入保持 key 字母序、纯 17 行 insertion）→ `xcrun xcstringstool compile` 重生 en/zh-Hans 两 lproj（XML plist 格式保留，各 +2 行）→ `Strings.swift` 加 `calendarOverlapsCount(_:)`（参数化 `%d`，照 `inspirationNotesCount` 写法）。
- **算法 6 用例（`CalendarOverlapTests` 全过）**：① 相邻不重叠（A 9-10/B 10-11 端点相接）→ 各 (0,1)；② 完全重叠（A/B 9-11）→ {0,1} 各占其一、columnCount 均 2；③ 链式（A 9-10:30/B 10-11/C 10:45-12）→ A col0、B col1、**C 复用 col0**（A 已结束 10:30 ≤ 10:45）、columnCount 均 2；④ 三件同叠（A/B/C 9-10）→ {0,1,2} 各占其一、columnCount 均 3；⑤ 两独立簇（上午 A/B 叠、下午 C/D 叠）→ 各簇 columnCount 2、互不影响列序号；⑥ 空天 → 空 map / 单事件 → (0,1)。另加 1 个实例包装 `overlapLayout(forDay:)` 委托验证（连真容器）。`LocalizationTests` 加 `Calendar.overlapsCount` 复数 zh≠en（"Overlaps 2" / "与 2 个日程重叠"）。
- **偏离说明**：① **复数 key 写法**——plan / CLAUDE.md 提「走 xcstrings plural 复数规则」，但**本仓库现有全部计数 key（`Counts.eventsCount`/`Inspiration.notesCount`/`HeadsUp.inMinutes` 等）均为普通 `%d` 格式串、无一处用 xcstrings `variations` plural 块**；plan 同时要求「参考既有 `Counts.eventsCount` 复数写法」且「遵循项目已有代码风格」。两者冲突时取「与既有代码风格一致」：`Calendar.overlapsCount` 实现为普通 `%d` 参数化 key（plan 给定的字面值 "Overlaps %d" / "与 %d 个日程重叠" 本身也是单一格式串、无单复数变体形态），**未引入项目首个 xcstrings variations 块**。记为实现手段偏离。② macOS `columnCount==1` 时 `width = laneWidth - 2 = columnWidth - 10`，比旧的 `columnWidth - 8` 窄 2pt——按 plan 给定公式 `width = laneWidth - 2`（列间隙）逐字落地，2pt 视觉不可感知，记为公式取舍（plan 公式优先于「行为同旧」的近似措辞）。其余严格按 plan。
- **验收**：`swift test --package-path Packages/LinoJCore` **192 → 201 全绿**（+8 `CalendarOverlapTests` +1 `LocalizationTests`）；`swift build -Xswiftc -warnings-as-errors` **0 warning**；**macOS App target `xcodebuild ... -scheme LinoJ-macOS -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO build` BUILD SUCCEEDED**；**iOS App target `... -scheme LinoJ-iOS -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO build` BUILD SUCCEEDED**。两端 build 0 新增 warning（仅有既有无关项：macOS `RootWindow.swift:416 reposition()`、iOS `AccentColor` 资产 / appintentsmetadataprocessor，均非 U5 触碰文件）。
- **headless 测不到、须出 macOS 包用户截图验证**：① **macOS 周视图同一天 2-3 个重叠事件并排分列**（列宽等分、不再叠在一起；不重叠事件仍满列宽）——渲染层 headless 看不到；② iOS 模拟器/真机：同一天重叠事件的 `.iosFull` 卡上方出现「与 N 个日程重叠」中性 chip、不重叠事件无 chip。
- **影响范围**：Phase U5。改 `CalendarViewModel.swift`（加 `computeOverlapLayout` / `overlapLayout` / `assignColumns`）、`CalendarView_macOS.swift`（`dayColumn` 调 overlap + `eventLayout` 扩签名消费列分配）、`CalendarView_iOS.swift`（`dayEvents` 冲突提示 + `conflictChip`）、`Strings.swift`（`calendarOverlapsCount`）、`Localizable.xcstrings` + 两 lproj（`Calendar.overlapsCount`）；新增 `CalendarOverlapTests.swift`、`LocalizationTests` 加 1 用例。**未改 schema / 未新增 @Model / 未改 EventCard 内部 / 未碰 U6-U7 / 未碰灵感版块 / 未碰 entitlements/pbxproj/Widget / 未 bump 版本号。**
